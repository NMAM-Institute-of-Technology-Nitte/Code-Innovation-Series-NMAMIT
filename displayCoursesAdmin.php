...
<head>
  ...
  <title>EDUCARE | ALL COURSES</title>
  <meta charset="utf-8">
...
<body>
    ...
    <?php
      ...
      {
        echo("<h1>Courses</h1><br>");
        echo("
        <div class='row'>
        <input class='form-control col-12 col-sm-4 offset-sm-8' id='myInput' type='text' placeholder='Search..'
        style='width:30%;'>
        </div>
        
        <br>
        <table class='table table-responsive-lg' data-toggle='table' data-pagination='true' data-search='true'  data-fixed-columns='true'
        data-fixed-number='2'>");
        echo("<thead><tr><th scope='col'>Course name</th><th scope='col'>Course code</th><th scope='col'>Course cost</th><th scope='col'>Course Category</th></tr></thead>");
        while(($row = mysqli_fetch_row($result))!=null)
        {
          echo("<tbody id='myTable'><tr class='myrow '><td>".$row[0]."</td><td>
          ". $row[1]. "</td><td>" .$row[3]. "</td><td>" .$row[4]. "
          </td></tr></tbody>");
        }
          echo("</tbody></table>");
        }   
